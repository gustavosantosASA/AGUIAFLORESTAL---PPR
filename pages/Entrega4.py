...
with col1:
    st.title("📝 Editar 4º Entrega")

with st.container():
    st.markdown("### 🎯 Informações da Meta")
    st.markdown(f"**Meta:** `{row['Descrição Meta']}`")
    st.markdown(f"4º Entrega")

with st.form("form_avaliacao"):
    new_value = st.text_input("✏️ Editar", value=row['4º Avaliação'])
    ...
    if submitted:
        ...
        def update_cell_in_sheet(df, url, worksheet_name, row_index, new_value):
            ...
            worksheet.update_cell(row_index + 2, df.columns.get_loc('4º Avaliação') + 1, new_value)

        df.at[row_index, '4º Avaliação'] = new_value
    ...
    if st.button("📤 Enviar Documento"):
        ...
        try:
            ...
            def update_doc_link(df, url, worksheet_name, row_index, doc_link):
                ...
                sheet = client.open_by_url(url)
                worksheet = sheet.worksheet(worksheet_name)
                worksheet.update_cell(row_index + 2, df.columns.get_loc('Doc4') + 1, doc_link)

            df.at[row_index, 'Doc4'] = drive_link
...
if pd.notna(row.get("Doc4")) and "drive.google.com" in row["Doc4"]:
    st.markdown("---")
    st.markdown("### 👁️ Pré-visualização do Documento")

    import re
    match = re.search(r"/d/([a-zA-Z0-9_-]+)", row["Doc4"])
    if match:
