...
with col1:
    st.title("📝 Editar 5º Entrega")

with st.container():
    st.markdown("### 🎯 Informações da Meta")
    st.markdown(f"**Meta:** `{row['Descrição Meta']}`")
    st.markdown(f"5º Entrega")

with st.form("form_avaliacao"):
    new_value = st.text_input("✏️ Editar", value=row['5º Avaliação'])
    ...
    if submitted:
        ...
        def update_cell_in_sheet(df, url, worksheet_name, row_index, new_value):
            ...
            worksheet.update_cell(row_index + 2, df.columns.get_loc('5º Avaliação') + 1, new_value)

        df.at[row_index, '5º Avaliação'] = new_value
    ...
    if st.button("📤 Enviar Documento"):
        ...
        try:
            ...
            def update_doc_link(df, url, worksheet_name, row_index, doc_link):
                ...
                sheet = client.open_by_url(url)
                worksheet = sheet.worksheet(worksheet_name)
                worksheet.update_cell(row_index + 2, df.columns.get_loc('Doc5') + 1, doc_link)

            df.at[row_index, 'Doc5'] = drive_link
...
if pd.notna(row.get("Doc5")) and "drive.google.com" in row["Doc5"]:
    st.markdown("---")
    st.markdown("### 👁️ Pré-visualização do Documento")

    import re
    match = re.search(r"/d/([a-zA-Z0-9_-]+)", row["Doc5"])
    if match:
